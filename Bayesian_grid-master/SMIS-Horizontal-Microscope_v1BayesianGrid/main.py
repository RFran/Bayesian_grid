...
##--------------Scan---Grid---Bayesian----------------------------------------------------------------------------------------------------
    ...
    global facteur_t
    facteur_t =1.5   # We use facteur_t to set the threshold -> Threhold = facteut_t*baseline
    ...
    def Scan_process(self):                      # This function allow us to choose between Grid&bayesian or Bayesian
        ...
        if self.btn_scanAll.isChecked():
            ...
            def Scan(self):
                ...
                from statistics import mean
                devices = sb.list_devices()  # On cherche un spectrometer
                if devices == []:
                    info_sb = QtGui.QMessageBox.question(self, 'Spectrometer Information', "No spectrometer found, make sure USB cable is connected.")
